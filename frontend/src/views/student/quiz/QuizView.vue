  ...
  >
    ...
    <div class="question-navigation">
      <div data-cy="navigationButtons" class="navigation-buttons">
        <span
          :questionNumber="statementQuiz.answers.length"
          v-for="index in +statementQuiz.answers.length"
          v-bind:class="[
            'question-button',
      ...
      <span
        ...
        @click="confirmAnswer"
        v-if="questionOrder !== statementQuiz.answers.length - 1"
...
export default class QuizView extends Vue {
  ...
  currentQuestion: StatementQuestion | null = null;
  ...
  async created() {
    ...
    console.log('quiz_id ' + this.statementQuiz.currentQuestion);

    this.questionOrder = this.statementQuiz && this.statementQuiz.currentQuestion || 0;

    console.log('question order '+this.statementQuiz);

    try {
        if (this.statementQuiz)
          this.currentQuestion = await RemoteServices.getNextQuestion(this.statementQuiz.id);
  ...
  async increaseOrder() {
    if (this.questionOrder + 1 < +this.statementQuiz!.answers.length) {
      this.calculateTime();
    ...
    if (this.statementQuiz?.oneWay) {
      ...
      if (!!this.statementQuiz)
        this.currentQuestion = this.statementQuiz.questions[this.questionOrder]
    }
  }
  ...
  changeOrder(newOrder: number): void {
    if (!this.statementQuiz?.oneWay) {
      if (newOrder >= 0 && newOrder < +this.statementQuiz!.answers.length) {
        this.calculateTime();
  ...
  confirmAnswer() {
    if (
      ...
      this.questionOrder + 1 < +this.statementQuiz!.answers.length
    ) {
