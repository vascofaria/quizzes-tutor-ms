...
const microHttpClient1 = axios.create();
microHttpClient1.defaults.timeout = 100000;
microHttpClient1.defaults.baseURL =
  process.env.VUE_APP_MICRO_API_1 || 'http://localhost:8083';
microHttpClient1.defaults.headers.post['Content-Type'] = 'application/json';
microHttpClient1.interceptors.request.use(
  config => {
    if (!config.headers.Authorization) {
      const token = Store.getters.getToken;
...
microHttpClient1.interceptors.response.use(
  response => {
    if (response.data.notification) {
      if (response.data.notification.errorMessages.length)
        Store.dispatch(
...
const microHttpClient2 = axios.create();
microHttpClient2.defaults.timeout = 100000;
microHttpClient2.defaults.baseURL =
  process.env.VUE_APP_MICRO_API_2 || 'http://localhost:8084';
microHttpClient2.defaults.headers.post['Content-Type'] = 'application/json';
microHttpClient2.interceptors.request.use(
  config => {
    if (!config.headers.Authorization) {
      const token = Store.getters.getToken;
...
microHttpClient2.interceptors.response.use(
  response => {
    if (response.data.notification) {
      if (response.data.notification.errorMessages.length)
        Store.dispatch(
...
const microHttpClient3 = axios.create();
microHttpClient3.defaults.timeout = 100000;
microHttpClient3.defaults.baseURL =
  process.env.VUE_APP_MICRO_API_3 || 'http://localhost:8085';
microHttpClient3.defaults.headers.post['Content-Type'] = 'application/json';
microHttpClient3.interceptors.request.use(
  config => {
    if (!config.headers.Authorization) {
      const token = Store.getters.getToken;
...
microHttpClient3.interceptors.response.use(
  response => {
    if (response.data.notification) {
      if (response.data.notification.errorMessages.length)
        Store.dispatch(
...
export default class RemoteServices {
  ...
  static async getQuizByQRCode(quizId: number): Promise<StatementQuiz> {
    let client;
    if (Store.getters.getUser.id % 3 == 0){
      client = microHttpClient1;
    } else if(Store.getters.getUser.id % 3 == 1) {
      client = microHttpClient2;
    } else {
      client = microHttpClient3;
    }
    return client
      .get(`/quiz/${quizId}/get/${Store.getters.getUser.id}`)
      .then(response => {
        return new StatementQuiz(response.data);
      })
      .catch(async error => {
  ...
  static async getNextQuestion(quizId: number): Promise<StatementQuestion>{
    let client;
    if (Store.getters.getUser.id % 3 == 0){
      client = microHttpClient1;
    } else if(Store.getters.getUser.id % 3 == 1) {
    ...
    return client
      .get(`/quiz/${quizId}/getNextQuestion/${Store.getters.getUser.id}`)
      .then(response => {
  ...
  static submitAnswer(quizId: number, answer: StatementAnswer) {
    ...
    client.post(`/quizzes/${quizId}/submit`, answer).catch(error => {
      console.debug(error);
    });
  }
